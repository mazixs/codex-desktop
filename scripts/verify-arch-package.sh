...
run_smoke_test() {
    ...
    launch_command="mkdir -p ~/.cache ~/.config ~/.local/share && launch_rc=0 && env CODEX_BROWSER_USE_NODE_PATH=/usr/bin/node CODEX_ELECTRON_BUNDLED_PLUGINS_RESOURCES_PATH=/opt/codex-desktop/dist CODEX_ELECTRON_RESOURCES_PATH=/opt/codex-desktop/dist CODEX_NODE_REPL_PATH=/opt/codex-desktop/dist/node_repl NODE_REPL_NODE_PATH=/usr/bin/node XDG_SESSION_TYPE=x11 WAYLAND_DISPLAY='' timeout 25s xvfb-run -a ${LAUNCHER} >${smoke_log} 2>&1 || launch_rc=\$?; if [ \"\$launch_rc\" -ne 0 ] && [ \"\$launch_rc\" -ne 124 ]; then cat ${smoke_log}; exit 1; fi; if grep -Eq \"Electron runtime not found|Build output not found|Codex CLI not found|Product Electron binary not found|Desktop bootstrap failed\" ${smoke_log}; then cat ${smoke_log}; exit 1; fi; if grep -Eq \"bundled_plugins_reconcile_failed|bundled_plugins_marketplace_install_failed\" ${smoke_log}; then cat ${smoke_log}; exit 1; fi; if grep -Eq \"packaged[\\\":= ]+false|react devtools extension loaded\" ${smoke_log}; then cat ${smoke_log}; exit 1; fi; if ! grep -Eq \"packaged[\\\":= ]+true\" ${smoke_log}; then cat ${smoke_log}; exit 1; fi; if grep -Fq \"/opt/codex-desktop/dist/node_repl\" ${smoke_log}; then cat ${smoke_log}; exit 1; fi; if ! grep -Fq \"/opt/codex-desktop/node_modules/electron/dist/resources/node_repl\" ${smoke_log}; then cat ${smoke_log}; exit 1; fi"
