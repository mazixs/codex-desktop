...
Do not treat `./codex-linux-build/start.sh` after `pnpm run build` as a final runtime smoke test. That path runs the unpacked fallback with `app.isPackaged=false`; it is useful for local patch iteration but not for Browser Use, bundled plugin, skills, or release validation. Product validation should use the portable artifact or installed package smoke tests and must prove the launcher selects product `resources/node_repl`, exposes `resources/codex`, and does not leak inherited stale `CODEX_*` paths. The external Chrome plugin must not be added to the bundled marketplace unless `plugins/chrome/extension-host/linux/x64/extension-host` is present and executable.
